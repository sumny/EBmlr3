#' @title Explainable Boosting Machine Classification Learner
#'
#' @name mlr_learners_classif.ebm
#'
#' @description
#' Explainable boosting machine.
#' Uses `ExplainableBoostingClassifier` from `interpret` module.
#'
#' `n_jobs` is set to 1 instead of 2. `random_state` is set to `42` (like in `interpret`).
#'
#' @templateVar id classif.ebm
#' @template learner
#'
#' @references
...
#' @template example
LearnerClassifEBM = R6Class("LearnerClassifEBM",
  ...
  public = list(
    ...
    initialize = function() {
      ...
      super$initialize(
        ...
        properties = c("multiclass", "twoclass", "weights"),  # FIXME: importance
        packages = "EBmlr3",
        man = "EBmlr3::mlr_learners_classif.ebm"
      )
    }
  ),
  ...
  private = list(
    ...
    .predict = function(task) {
      ...
      if ("response" %in% self$predict_type) {
        response = self$model$predict(X = newdata)
